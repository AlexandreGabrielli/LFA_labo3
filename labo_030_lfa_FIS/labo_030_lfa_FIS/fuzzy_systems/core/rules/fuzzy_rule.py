...
class FuzzyRule:
    ...
    def fuzzify(self, crisp_inputs: Dict[str, float]) -> List[float]:
        ...
        for antecedent in self.antecedents:
            input_value = crisp_inputs[antecedent.lv_name.name]

            if antecedent.is_not:
                fuzzified = 1.0 - antecedent.lv_name[antecedent.lv_value].fuzzify(input_value)
            else:
                fuzzified = antecedent.lv_name[antecedent.lv_value].fuzzify(input_value)

            fuzzified_inputs_for_rule.append(fuzzified)
